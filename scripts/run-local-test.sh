...
export AC_PG_DSN=postgres://postgres:postgres@localhost:5432/postgres
export AC_PG_MIGRATION=../sql/migrations
export AC_ROOT_PASSWORD=123456
...
if [ -z "$1" ]; then
    ...
    go test -v -timeout 120s -run ^$1$ github.com/cloudcarver/anchor/e2e
    exit_code=$?
fi
