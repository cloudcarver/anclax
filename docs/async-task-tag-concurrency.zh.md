...
迁移 16 引入状态式预取：system 任务先为业务任务申请全部配额并设置 `ready`，Worker 再批量接管为 `running`，不重复扫描 pending 或申请 tag 名额。`ready` 占用配额，没有有效期和固定数量上限，取消、暂停或调度属性变化时原子撤销。system 任务独立领取，TaskStore 不接受其 tag、serial 或自定义 weight 配置。完整状态机、升级和回滚见[预取设计](ready-task-prefetch.md)。
...
ready 是持久化的预计算准入结果，没有有效期，也不按 Worker 并发或 4,096 条限制数量。预占的资源在接管时转交给执行阶段，或由取消、暂停、调度属性变化等操作释放。执行任务继续使用独立租约。完整恢复和升级语义见[预取设计](ready-task-prefetch.md)。

预取 system 任务长期运行，每批使用独立短事务。按资源／路由／普通或 strict 类别分组观察 ready 存量及消费速度，以约 500 ms 消费量和至少一批 256 条作为软储备目标；储备充足时完全停止该组的候选入队计算，储备不足则连续补充。这不是入队数量上限，已有 ready 不会失效。观察查询和执行租约回收继续运行，领取路径不写消费计数。Worker 保留两个控制槽位，让其他 system 命令持续执行。简单任务批量更新为 ready，受限任务和过期执行租约保留完整申请与释放路径。
