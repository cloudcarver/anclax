...
`TestBatchedTaskLeaseRenewalSmoke` verifies batch fencing and renewal/cancellation while the business pool is fully occupied. For sustained concurrency measurements with constrained pools, run `make taskcore-capacity`; see [sustained capacity methodology and results](scheduling-capacity-benchmark.md).
