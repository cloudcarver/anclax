...
> 📚 **需要技术细节？** 查看[技术参考](async-tasks-technical.zh.md)了解底层架构、生命周期和内部机制。
>
> ⚖️ **需要调度控制？** 查看[调度与运行时配置指南](async-task-scheduling-runtime-config.zh.md)，了解 `WithPriority`、`WithWeight`、strict-cap 和在线配置更新。
...
- [如何编写任务代码](#如何编写任务代码)
- [如何启动任务](#如何启动任务)
- [Priority、Weight 与运行时配置](#priorityweight-与运行时配置)
...
任务在 `api/tasks/tasks.yaml` 中使用结构化的 YAML 格式定义：
...
### Priority、Weight 与运行时配置

Anclax 支持两条调度通道：

- **严格通道**：`priority > 0`。
- **普通通道**：`priority == 0`，按标签组加权公平调度。

#### 任务级调度覆盖

```go
_, err := h.taskRunner.RunTaskName(ctx, params,
    taskcore.WithPriority(10), // 严格通道（紧急）
    taskcore.WithWeight(3),    // 在选中普通组中更靠前
)
```

校验规则：
- `WithPriority(priority)` 要求 `priority >= 0`。
- `WithWeight(weight)` 要求 `weight >= 1`。
