...
The historical [arrival and backoff follow-up](arrival-prefetch-benchmark.md) measures the consumption scheduler and independent empty-result backoff using persistent Workers with independently scheduled flow ramps/bursts, mixed durations and hot resources, and stopped-consumption recovery. Those exploratory harnesses have been retired; their reports remain. The maintained benchmark is the [sustained capacity benchmark](scheduling-capacity-benchmark.md), focused on maximum healthy execution concurrency with minimum resources.
