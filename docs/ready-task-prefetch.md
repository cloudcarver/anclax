...
The long scheduler observes indexed ready counts and due pending work separately for each resource/routing/priority-class group. Due eligibility excludes routes without a compatible live Worker and groups with a zero quota; later Worker/configuration changes are observed even while admission is paused. Between observations it adds its own committed admissions to predicted stock. The decrease in that stock estimates consumption without a claim-side counter write or shared accounting row. Cancellation and edits can also remove ready rows, conservatively overestimating reserve demand; the estimate never grants resources or authorizes ownership. Removed/empty groups are discarded from local controller state, and takeover reconstructs it from PostgreSQL.

The current policy targets 500 ms of observed consumption, with one 256-task batch as a bootstrap/minimum reserve. This is a soft scheduling target, never a SQL limit or a reason to reject/revoke ready work. Rate increases take effect immediately and decay exponentially with a one-second time constant. An empty due group always permits refill even with zero measured consumption, preventing a supply shortage from imposing a lower refill rate. Successive productive batches continue immediately until the estimated reserve is sufficient. Groups with sufficient stock are excluded from candidate SQL, so one slow stocked route cannot suppress another group's arrivals or consumption.

Observations target 50 ms while due work exists and up to 100 ms when idle; the maintenance deadline can advance the next observation. If every group is paused, no candidate/admission SQL runs. Empty admission with active resource owners retries at 5 ms; idle/quiescent empty admission backs off through 20, 40, 80 and 100 ms. These intervals are start-to-start policy targets, not completion-latency guarantees. No notification mechanism is introduced.
