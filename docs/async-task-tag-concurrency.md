...
The prefetch task keeps one execution lease across bounded transactions and immediately follows productive batches to fill the existing ready window. Full supply or due work with active resource owners receives a short 5 ms check; only idle/quiescent rounds back off to 100 ms. No consumption credits or claim-side demand counters are used. Two control slots preserve system-command progress. Fresh tasks without finite tags or serial keys enter ready through one bulk update, retaining unlimited-tag snapshots for quota backfill; constrained tasks and expired leases use the full allocator.
