...
The prefetch task keeps one execution lease across bounded batch transactions. It paces admission by committed automatic consumption, skips heavy admission without refill credit, and backs off empty attempts to 250 ms with periodic recovery/arrival probes. Per-Worker demand counters are independent of tag quotas; unlimited tags remain uncounted. Two reserved control slots keep other system commands progressing alongside it. Fresh tasks without finite tags or serial keys enter ready with one bulk update; unlimited tag snapshots are retained for future quota backfill. Constrained tasks and expired leases keep the full allocation/release path.
