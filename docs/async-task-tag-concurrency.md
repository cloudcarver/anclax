...
Ready is durable precomputed admission: it has no expiry or fixed quantity limit. Resource reservations transfer to execution on claim, or are revoked by cancellation, pause or scheduling edits. Execution leases remain independent. See the [prefetch design](ready-task-prefetch.md) for recovery and integration.
...
The long-lived prefetch task observes ready stock and consumption separately by resource/routing/priority-class group. A 500 ms consumption reserve with a bootstrap batch of 256 is a soft scheduling target, not a queue limit. Candidate SQL stops for stocked groups; bounded productive batches refill other groups immediately. Fenced observations and execution-lease recovery continue while admission is paused, without claim-side accounting writes. Two control slots preserve other system commands. Simple tasks use bulk admission; constrained tasks and expired execution owners retain full allocation/release.
