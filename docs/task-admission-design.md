...
The long scheduler uses committed per-Worker ready consumption to control refill volume and cadence. It retains bounded initial fill, empty backoff and periodic probes for zero-consumption recovery and new arrivals. Demand counters are hints, with the existing PostgreSQL allocation protocol and ready cap remaining authoritative. See [consumption pacing](ready-task-prefetch.md#consumption-pacing) for the small-quota feedback safeguard and latency tradeoff.
