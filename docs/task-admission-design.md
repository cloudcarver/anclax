...
Migration 16 adds a durable `ready` phase in the existing task table. A reserved system lane directly executes the singleton `prefetchTasks` job, which performs bounded pending admission and expiry recovery. Resource-group indexes let it skip a full limited-tag combination as a unit. Successful admission reserves slots and serial ownership before publishing ready rows. Unlimited tags do not split otherwise identical routing groups.
