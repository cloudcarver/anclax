...
The [automatic admission comparison](async-task-admission-benchmark.md) reproduced a local baseline claim/finalize deadlock involving the same SQL pair and tag-counter relation as the production report, with a saved PostgreSQL lock-cycle log. It does not reconstruct the exact production transaction schedule. The PR passed all 24 measured workload cases and a 200-iteration container chaos run; finite hot tags still incurred recovered guard retries and wakeup latency. These tests do not establish production capacity. Production must be re-baselined after upgrade. Old/new protocol versions must not run together during migration or rollback; see [upgrade instructions](async-task-tag-concurrency.md#storage-compatibility-and-upgrade).
