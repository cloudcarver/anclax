...
VALUES ($1)
RETURNING version, payload, created_at, request_id
`
...
const createWorkerRuntimeConfigForRequest = `-- name: CreateWorkerRuntimeConfigForRequest :one
...
type CreateWorkerRuntimeConfigForRequestParams struct {
	RequestID *string
	Payload   json.RawMessage
}

func (q *Queries) CreateWorkerRuntimeConfigForRequest(ctx context.Context, arg CreateWorkerRuntimeConfigForRequestParams) (*AnclaxWorkerRuntimeConfig, error) {
	row := q.db.QueryRow(ctx, createWorkerRuntimeConfigForRequest, arg.RequestID, arg.Payload)
	var i AnclaxWorkerRuntimeConfig
	err := row.Scan(
		&i.Version,
		&i.Payload,
...
SELECT version, payload, created_at, request_id FROM anclax.worker_runtime_configs
